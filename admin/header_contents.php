...
<?php 
    ...
    if(!defined("BASE_PATH")) {
        $current_dir = __DIR__;
        // admin/header_contents.php is 1 level deep from root
        define("BASE_PATH", dirname($current_dir) . '/');
    }
    
    include_once BASE_PATH . 'security/config.php';
    include_once BASE_PATH . 'security/constants.php';
    
    // Use Logo as Favicon for consistent branding
    $favicon_src = (strpos($APP_LOGO, 'http') === 0) ? $APP_LOGO : BASE_PATH . $APP_LOGO;
    if(empty($APP_LOGO) || $APP_LOGO == "wincologo.png") $favicon_src = BASE_PATH . "favicon.ico";
?>
