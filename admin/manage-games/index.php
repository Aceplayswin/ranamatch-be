...
<head>
    ...
    <style>
        ...
        .status-toggle-premium:hover {
            border-color: var(--accent-blue);
            box-shadow: 0 0 12px rgba(16, 185, 129, 0.15);
        }

        .status-toggle-premium.active {
        ...
        .status-toggle-premium.active .toggle-knob {
            left: 20px !important;
        }
        ...
        .premium-row:hover .game-icon-wrapper {
            border-color: var(--accent-blue);
        ...
        .premium-row.sortable-ghost {
            opacity: 0.4;
            background: var(--table-row-hover) !important;
        }

        .premium-row.sortable-chosen {
            background: var(--table-row-hover) !important;
            box-shadow: var(--card-shadow);
        }
...
<body class="bg-light">
    <div class="admin-layout-wrapper">
        ...
        <div class="admin-main-content">
            <!-- Accent Bar -->
            <div
                style="height: 2px; background: linear-gradient(90deg, #06b6d4, #3b82f6, #8b5cf6, #3b82f6, #06b6d4); background-size: 300% 100%; animation: gradientShift 6s ease infinite; border-radius: 4px; margin-bottom: 12px; opacity: 0.5;">
            </div>
            ...
            <div class="stats-container">
                ...
                <div class="glass-card stat-card-inner" style="animation-delay: 0.1s;">
                    <div class="stat-icon"
                        style="background: linear-gradient(135deg, rgba(6,182,212,0.15), rgba(59,130,246,0.15)); color: #06b6d4;">
                ...
                <div class="glass-card stat-card-inner" style="animation-delay: 0.2s;">
                    <div class="stat-icon"
                        style="background: linear-gradient(135deg, rgba(16,185,129,0.15), rgba(52,211,153,0.15)); color: #10b981;">
                ...
                <div class="glass-card stat-card-inner" style="animation-delay: 0.3s;">
                    <div class="stat-icon"
                        style="background: linear-gradient(135deg, rgba(251,191,36,0.15), rgba(245,158,11,0.15)); color: #fbbf24;">
                ...
                <div class="glass-card stat-card-inner" style="animation-delay: 0.4s;">
                    <div class="stat-icon"
                        style="background: linear-gradient(135deg, rgba(168,85,247,0.15), rgba(139,92,246,0.15)); color: #a855f7;">
            ...
            <div class="glass-card"
                style="padding: 12px 16px; margin-bottom: 16px; display: flex; justify-content: space-between; align-items: center; border-left: 3px solid #3b82f6; animation: fadeInUp 0.65s ease both;">
                <div class="listing-info-premium" style="display: flex; align-items: center; gap: 8px;">
                    ...
                    <span style="font-size: 12px;">Showing <b
                            style="color: #3b82f6;"><?php echo mysqli_num_rows($result); ?></b> Game Records</span>
                </div>
                <div style="display: flex; gap: 6px;">
                    <button class="btn-ghost" onclick="exportToCSV()"
                        style="height: 28px !important; font-size: 9px !important; padding: 0 8px !important;">
                        ...
                        while ($row = mysqli_fetch_assoc($result)):
                            ...
                            <tr class="premium-row" data-id="<?php echo $row['id']; ?>">
                                ...
                                <td style="color: var(--text-dim); font-weight: 700;">
                                    <?php echo str_pad($no++, 2, '0', STR_PAD_LEFT); ?></td>
                                <td>
                                    <div style="display: flex; align-items: center; gap: 10px;">
                                        <div class="game-icon-wrapper">
                                            <img src="<?php echo htmlspecialchars($row['game_image']); ?>"
                                                class="game-icon-premium"
                                                onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
                                            <div class="img-fallback-premium"><i class='bx bx-image'></i></div>
                                        </div>
                                        <div>
                                            <div style="font-weight: 800; color: var(--text-main); margin-bottom: 1px;">
                                                <?php echo $row['game_name']; ?></div>
                                            <div style="font-size: 9px; color: var(--text-dim); font-family: monospace;">
                                                <?php echo $row['game_uid']; ?></div>
                                        </div>
                                    </div>
                                </td>
                                <td>
                                ...
                                <td style="text-align: center;">
                                    <div style="display: flex; justify-content: center;">
                                        <div class="status-toggle-premium <?php echo $row['game_status'] == 1 ? 'active' : ''; ?>"
                                ...
                                <td style="text-align: center;">
                                    <input type="text" class="sort-num-input" value="<?php echo $row['sort_order']; ?>"
                                        readonly
                                        style="width: 34px; height: 24px; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); border-radius: 4px; color: var(--accent-blue); text-align: center; font-size: 10px; font-weight: 700; outline: none; cursor: default;">
                                </td>
                                <td>
                                    <div style="display: flex; gap: 6px; justify-content: center;">
                                        <button class="action-btn-premium edit"
                                            onclick="editGame(<?php echo htmlspecialchars(json_encode($row)); ?>)">
                                            <i class='bx bx-edit-alt'></i>
                                        </button>
                                        <button class="action-btn-premium delete"
                                            onclick="deleteGame(<?php echo $row['id']; ?>)">
    ...
    <div id="bulkModal" class="modal-overlay">
        <div class="modal-content">
            ...
            <div class="form-group">
                ...
                <textarea id="bulkJsonInput" class="cus-inp"
                    style="height: 450px; resize: vertical; padding: 12px; font-family: 'Consolas', monospace; font-size: 12px; line-height: 1.5; white-space: pre;"
                    placeholder='[
  {
    "name": "Game Name",
    "uid": "game_uid",
  ...
  ...
    ...
    <script>
        ...
        function toggleRoulette(id, el) {
            const current = el.classList.contains('active') ? 0 : 1;
            fetch('update_logic.php', {
                method: 'POST',
                body: new URLSearchParams({ action: 'toggle_roulette', id: id, is_roulette: current })
            })
                .then(res => res.json())
                .then(data => {
                    if (data.success) {
                        el.classList.toggle('active');
                        Swal.fire({ icon: 'success', title: 'Roulette Status Updated', toast: true, position: 'top-end', showConfirmButton: false, timer: 1500 });
                    }
                });
        }
        ...
        function exportToCSV() {
            let csv = 'No,Name,UID,Category,Provider,Status,Featured,Roulette\n';
            const rows = document.querySelectorAll('.premium-row');
            rows.forEach((row, index) => {
                ...
                const roulette = row.querySelectorAll('.feature-star')[1].classList.contains('active') ? 'Yes' : 'No';
                csv += `${index + 1},"${name}","${uid}","${cat}","${prov}","${status}","${featured}","${roulette}"\n`;
            });
        ...
        function exportToExcel() {
            ...
            rows.forEach((row, index) => {
                data.push({
                    ...
                    'Roulette': row.querySelectorAll('.feature-star')[1].classList.contains('active') ? 'Yes' : 'No'
                });
            });
        ...
        function exportToPDF() {
            ...
            document.querySelectorAll('.premium-row').forEach((row, index) => {
                rows.push([
                    ...
                    row.querySelectorAll('.feature-star')[1].classList.contains('active') ? 'Yes' : 'No'
                ]);
            });

            doc.autoTable({
                head: [['No', 'Game Name', 'UID', 'Category', 'Provider', 'Status', 'Featured', 'Roulette']],
                body: rows,
                startY: 35,
                theme: 'grid',
